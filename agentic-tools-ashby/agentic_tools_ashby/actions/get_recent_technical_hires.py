...
from agentic_tools_ashby.client import _resolve_technical_keywords
from agentic_tools_ashby.bootstrap import tool
from agentic_tools_ashby.runtime_clients import get_ashby_client

SOURCE = "app/tools/ashby/actions/get_recent_technical_hires.py"
...
@tool(
    ToolDefinition(
        tool_id="ashby.get_recent_technical_hires",
        display_name="Get Recent Technical Hires",
        source_path=SOURCE,
        function_name="run",
        owner="recruiting-platform",
        version="2.3.0",
        description=(
            "Pull recent technical hires from Ashby with deterministic default semantics "
        ...
        examples=[
            "count=10, role_context='backend kernel engineer'",
            "count=20, keywords=['cuda', 'kernel'], retrieval_policy='strict_count'",
            "count=5, sort_by='created_at', sort_order='desc'",
        ],
...
def run(payload: dict[str, Any], preview: bool = False) -> dict[str, Any]:
    ...
    result = get_ashby_client().search_hires(
        ...
        filters={
            "status": ["hired"],
            "keywords": _resolve_technical_keywords(data.role_context, data.keywords),
            "technical_only": True,
    ...
    return {
        ...
        "summary": (
            f"Loaded {len(normalized.hires)} recent technical hires from Ashby "
